...
def export_to_kml(gdf, kml_path, polygon=None, polygon_crs=None):
    """Ekspor GeoDataFrame ke file KML langsung"""
    gdf_wgs = gdf.to_crs("EPSG:4326")
    k = kml.KML()
    doc = kml.Document(ns='', name='Roads Export', description='Generated by Road Converter')
    k.append(doc)
    ...
    for _, row in gdf_wgs.iterrows():
        ...
        p = kml.Placemark(ns='', name=layer, description=hwy)
        p.geometry = geom
        doc.append(p)

    # tambahkan batas area (optional)
    if polygon is not None and polygon_crs is not None:
        poly = gpd.GeoSeries([polygon], crs=polygon_crs).to_crs("EPSG:4326").iloc[0]
        boundary_pm = kml.Placemark(ns='', name='Boundary', description='Input area boundary')
        boundary_pm.geometry = poly
        doc.append(boundary_pm)
