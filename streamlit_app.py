...
def reverse_geocode(lat, lon):
    try:
        ...
        if response.status_code == 200:
            ...
            subdistrict = address.get("suburb", "") or address.get("village", "")
            district = address.get("city_district", "") or address.get("county", "") or address.get("city", "")
            return postalcode, subdistrict, district
    except:
        pass
    return "", "", ""
...
if kmz_file and template_file:
    ...
    # Ambil koordinat pertama HP COVER untuk keperluan reverse geocoding
    if hp_list:
        sample_hp = hp_list[0]
        postalcode, subdistrict, district = reverse_geocode(sample_hp["lat"], sample_hp["lon"])
    else:
